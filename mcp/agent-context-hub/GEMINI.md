...
**What**: Scans your `agents` folder and finds all available agents  
**Why**: No manual registration needed - just add an agent folder and it's discovered  
**How**: Reads each agent's `agent.json` and `agent.md` files on startup
...
Add to your `settings.json` or MCP client config:
