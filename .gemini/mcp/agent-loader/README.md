...
- **load_agent_context**: Loads agent-specific GEMINI.md context
...
The MCP server is automatically started by Gemini CLI when configured in `.gemini/settings.json`.

Example configuration for `.gemini/settings.json`:
...
### `load_agent_context`
Loads the `GEMINI.md` file for a specific agent.
