...
```json
{
  "mcpServers": {
    ...
    "agent-execution-hub": {
      "command": "bun",
      "args": [".gemini/mcp/agent-execution-hub/src/index.ts"],
      "env": {
...
```bash
cd .gemini/mcp/agent-execution-hub
echo '{"jsonrpc":"2.0","id":1,"method":"tools/list"}' | bun src/index.ts
```
...
```bash
ls .gemini/agents/github-manager/
```
...
1. Add more specialized agents to `.gemini/agents/`
2. Create workflow templates in `.gemini/mcp/agent-context-hub/workflows/templates/`
...
- [Agent Context Hub Docs](../agent-context-hub/GEMINI.md)
- [Agent Execution Hub Docs](../agent-execution-hub/GEMINI.md)
