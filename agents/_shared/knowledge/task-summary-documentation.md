...
After completing ANY task, you MUST create a summary in the `_summaries/` folder.
...
```
_summaries/YYYY-MM-DD/task-description.md
```

**Examples:**
- `_summaries/2025-10-27/fix-login-authentication-bug.md`
- `_summaries/2025-10-27/implement-user-dashboard.md`
- `_summaries/2025-10-28/research-oauth2-implementation.md`
...
1. **Check if today's folder exists**
   ```bash
   ls -la _summaries/ | grep $(date +%Y-%m-%d)
   ```

2. **Create folder if it doesn't exist**
   ```bash
   mkdir -p _summaries/$(date +%Y-%m-%d)
   ```

3. **Create summary file**
   ```bash
   # File goes in today's folder
   _summaries/2025-10-28/your-task-name.md
   ```

4. **Use file metadata to find summaries by date**
   ```bash
   # List all summaries from a specific date
   ls -la _summaries/2025-10-27/

   # Find summaries from last 7 days
   find _summaries/ -type f -name "*.md" -mtime -7

   # Sort all summaries by modification time
   find _summaries/ -type f -name "*.md" -exec ls -lt {} +
   ```
...
3. **Place in `_summaries/YYYY-MM-DD/` folder** organized by date
...
[Create file: _summaries/2025-10-27/task-name.md]

Summary created: _summaries/2025-10-27/task-name.md ✅
```
...
- [ ] File is saved in `_summaries/YYYY-MM-DD/` folder
